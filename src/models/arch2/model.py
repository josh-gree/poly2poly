...
class poly2poly(object):
    ...
    def __init__(self, sess, model_name, train_path, test_path, val_path):
        ...
        self.model_name = model_name + \
            '_{}_{}_{}_{}'.format(now.tm_mday, now.tm_mon,
    ...
    def model(self, x):
        ...
        d1, d1_w, d1_b = deconv2d(
            lrelu(e5_b), [self.batch_size, 32, 32, 1024], name='g_d1', with_w=True)
        d1 = self.g_bn_d1a(d1)
        ...
        d2, d2_w, d2_b = deconv2d(
            lrelu(d1_b), [self.batch_size, 64, 64, 512], name='g_d2', with_w=True)
        d2 = self.g_bn_d2a(d2)
        ...
        d3, d3_w, d3_b = deconv2d(
            lrelu(d2_b), [self.batch_size, 128, 128, 256], name='g_d3', with_w=True)
        d3 = self.g_bn_d3a(d3)
        ...
        d4, d4_w, d4_b = deconv2d(
            lrelu(d3_b), [self.batch_size, 256, 256, 128], name='g_d4', with_w=True)
        d4 = self.g_bn_d4a(d4)
        ...
        for k in range(epochs):
            ...
            for i in range(self.dataset_sx_tr):
                ...
                if i % 50 == 0:
                    ...
                    print(
                        '[iteration {}] -> val error: {}'.format(i, self.val_error()))
